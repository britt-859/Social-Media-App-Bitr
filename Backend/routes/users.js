...
route.get("/:user_id", async (req, res) => {
  ...
  try {
    const { payload: user, error } = await userService.getUser(
      Number.parseInt(req.params.user_id)
    );
...
route.delete("/:user_id", async (req, res) => {
  ...
  try {
    const { payload, error } = await usersService.deleteUser(
      Number.parseInt(req.params.user_id)
    );
...
route.put("/:user_id", async (req, res) => {
  ...
  try {
    const { payload: updatedUser, error } = await usersService.updateUser(
      req.params.user_id,
      req.body
    );
